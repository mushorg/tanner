...
class TestAPIServer(AioHTTPTestCase):
    ...
    async def test_api_snares_request(self):
        ...
        request = await self.client.request("GET", "/snares")
        assert request.status == 200
        detection = await request.json()
        self.assertDictEqual(detection, assert_content)
    ...
    async def test_api_sessions_request(self):
        ...
        request = await self.client.request("GET",
                                            "/8fa6aa98-4283-4085-bfb9-a1cd3a9e56e4/sessions?filters=peer_ip:127.0.0.1 start_time:1497890400 user_agent:ngnix")  # noqa
        assert request.status == 200
        detection = await request.json()
        self.assertDictEqual(detection, assert_content)
    ...
    async def test_api_sessions_info_request(self):
        ...
        request = await self.client.request("GET", "/session/4afd45d61b994d9eb3ba20faa81a45e1")
        assert request.status == 200
        detection = await request.json()
        self.assertDictEqual(detection, assert_content)
